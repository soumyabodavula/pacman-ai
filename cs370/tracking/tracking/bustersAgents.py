...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            tempGhostPos = max(dist, key=lambda key: dist[key]) # most likely ghost pos in this dist
            ghostDist = util.manhattanDistance(tempGhostPos, pacmanPosition)
            if ghostDist < closestGhostDist: # find closest most likely ghost
                closestGhostDist = ghostDist
                closestGhostPos = tempGhostPos

        closestAction = None
        closestDist = sys.maxsize
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            nextDist = util.manhattanDistance(nextPos, closestGhostPos)
