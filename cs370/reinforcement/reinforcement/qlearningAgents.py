...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for a in actions:
          qvalue = self.getQValue(state, a)
          if max_value == None or max_value < qvalue:
            max_value = qvalue
            max_action = a
    ...
    def getAction(self, state):
        ...
        if boolean: # random action
            return random.choice(legalActions)          
        else: # follows current best Q-values otherwise
    ...
    def update(self, state, action, nextState, reward):
        ...
        curr_value = self.qvalues[(state, action)]
        next_qvalue = self.computeValueFromQValues(nextState)
        difference = reward + (self.discount * next_qvalue) - curr_value
        self.qvalues[(state, action)] = curr_value + (self.alpha * difference) 
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        difference = reward + self.discount*next_value - curr_value
